...
with st.sidebar:
    # Banner caminhão
    st.image(str(BASE_DIR / "caminhão.png"), use_container_width=True)
    # Logo G3
    col_logo, col_txt = st.columns([1, 2])
    with col_logo:
        st.image(str(BASE_DIR / "logo.jpg"), width=60)
    with col_txt:
        st.markdown("**De olho no custo**")
        st.caption("Manutenção & orçamento")
    ...
    data_path = str(BASE_DIR / "orcamento.xlsx")
    if uploaded:
    ...
    st.markdown("---")
    st.caption("G3 Gestão · v4.0 · ● Sistema ativo")
...
if page == "📊 Dashboard":
    ...
    # Hero banner
    truck_b64 = img_b64(str(BASE_DIR / "caminhão.png"), "png")
    st.markdown(f"""
    ...
    with c8:
        st.markdown("**📊 Ciclo de Manutenção**")
        st.image(str(BASE_DIR / "diagrama.jpg"), use_container_width=True)
        st.caption("Engenharia · Planejamento · Execução · Controle")
